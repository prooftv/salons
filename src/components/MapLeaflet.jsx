...
function MapLeaflet(props) {
  ...
  function SetBounds() {
    ...
    const markerBounds = latLngBounds(markers)
  ...
  useEffect(() => {
    tooltipRefs.current &&
      tooltipRefs.current.forEach((item) =>
        item &&
        item._contentNode.classList.contains(`map-custom-tooltip-${hover || props.hoverCard}`)
  ...
  return (
    <MapContainer
      ...
      tap={props.tap}>
      <Focus />
      <TileLayer url={tileLayers[0].tiles} attribution={tileLayers[0].attribution} />
      {props.geoJSON &&
        props.geoJSON.features &&
              ...
              <Popup className="map-custom-popup" maxWidth="600" minWidth="200">
                <div className="popup-venue">
                  ...
                  <div className="text">
                    ...
                    {data.address && <p className="text-muted mb-1">{data.address}</p>}
                    {data.email && (
                      <p className="text-muted mb-1">
                        <FontAwesomeIcon icon={faEnvelopeOpen} className="fa-fw text-dark me-2" />
                        <a href={`mailto:${data.email}`}>{data.email}</a>
                      </p>
                    )}
                    {data.phone && (
                      <p className="text-muted mb-1">
                        <FontAwesomeIcon icon={faPhone} className="fa-fw text-dark me-2" />
                        {data.phone}
                      </p>
                    )}
                  </div>
      ...
      {props.markerPosition && <Marker position={props.markerPosition} icon={icon} />}
