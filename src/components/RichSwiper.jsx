...
function RichSwiper(props) {
  ...
  return props.data ? (
    <Swiper
              ...
              <div className="w-100">
                <Row>
                  <Col lg={slide.blocks ? "12" : "6"}>
                    ...
                    <h2 style={{ lineHeight: "1" }} className="display-3 fw-bold mb-3">
                      {slide.title}
                    </h2>
                    <p className="mb-5">{slide.content}</p>
                    {slide.button && (
                      <Link href={slide.buttonLink} passHref>
                        <Button variant="outline-light" className={slide.buttonClasses}>
                          {slide.button}
                  ...
                  <Col lg={slide.iconsRight ? "6" : "12"} className="ps-lg-5 my-3 my-md-5 my-lg-0">
                    {slide.iconsRight &&
                      slide.iconsRight.map((icon) => (
                        <a
                          ...
                          <div className="icon-rounded bg-white opacity-7 flex-shrink-0 me-4">
                            <Icon icon={icon.icon} className="text-dark w-2rem h-2rem" />
                          </div>
                          <div>
                            <h5>{icon.title}</h5>
                            {icon.badge && (
                ...
                {slide.blocks && (
                  <Row>
                    {slide.blocks.map((block) => (
                      <Col key={block.title} md="4" className="d-none d-md-block mb-5">
                        <Card className="h-100 border-0 shadow-lg bg-dark hover-animate">
                          ...
                          <Card.Body className=" d-flex align-items-center">
                            <div className="w-100">
                              <Card.Title as="h6">
                                <Link href={block.link}>
                                  <a className="text-decoration-none text-white">{block.title}</a>
                                </Link>
                              </Card.Title>
      ...
      <div
        ref={paginationRef}
        className={`swiper-pagination ${props.paginationClass ? props.paginationClass : ""}`}
      />
    </Swiper>
  ) : (
    "loading"
